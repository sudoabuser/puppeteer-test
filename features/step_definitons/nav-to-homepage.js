...
Given('Bob is on the internet', async () => {
    ...
    if (response.status() == 200) { // look for HTML response. 200 means we're connected.
        ...
        throw new Error("Bob's lost!!: Response status is not 200")
    }
});
...
Then('Bob should see the modanisa homepage', async () => {
    ...
    const logoTitle = await ppt.page.evaluate(() => {
        ...
        return logo ? logo.getAttribute('title') : null; //return title of the logo, if not available then return null
    });

    try {
        assert.strictEqual(logoTitle, "Modanisa") // check if modanisa and logotitle equal
    } catch (error) {
